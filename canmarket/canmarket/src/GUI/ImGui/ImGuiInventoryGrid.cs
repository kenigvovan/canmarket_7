...
namespace canmarket.src.GUI
{
    public class ImGuiInventoryGrid
    {
        ...
        /// <summary>Draw a single slot at the current cursor position, then advance cursor.</summary>
        ...
        private void DrawSlotAt(int slotId, Vector2 pos, int size, Vector4 tint = default)
        {
            ...
            // All visuals go through DrawList — cursor is never touched by them
            _slotRenderer.DrawSlotBackground(pos, dl);

            if (tint.W > 0f)
                dl.AddRectFilled(pos, pos + new Vector2(size, size), ImGui.GetColorU32(tint), 2f);

            // InvisibleButton is the only real ImGui item — owns the layout space
