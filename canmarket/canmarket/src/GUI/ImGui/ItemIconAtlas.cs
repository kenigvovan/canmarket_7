...
namespace canmarket.src.GUI
{
    public class ItemIconAtlas : IDisposable
    {
        ...
        {
            var attrs = new FramebufferAttrs("canmarket-item-atlas", AtlasSize, AtlasSize);
        ...
        /// <summary>Draw icon via DrawList — does not affect ImGui cursor position.</summary>
        public void DrawToList(ItemStack stack, Vector2 pos, Vector2 size, ImDrawListPtr dl)
        {
            if (stack == null || stack.Collectible == null) return;
            string key = GetKey(stack);
            if (!_itemIndexMap.TryGetValue(key, out int index))
            ...
            GetUv(index, out var uv0, out var uv1);
            dl.AddImage((IntPtr)TextureId, pos, pos + size, uv0, uv1);
        }
